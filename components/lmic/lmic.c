...
static bit_t decodeFrame (void) {
    ...
    if( dlen < OFF_DAT_OPTS+4 ||
      ...
      norx:
        ...
        printf("%lu: Invalid downlink, window=%s\n", (unsigned long int) os_getTime(), window);
#endif
        LMIC.dataLen = 0;
        return 0;
    }
    ...
    if( dlen > LEN_JA ) {
        ...
        for( u1_t chidx=3; chidx<8; chidx++, dlen+=3 ) {
            ...
            if( freq ) {
                ...
                printf("%lu: Setup channel, idx=%d, freq=%lu\n", (unsigned long int) os_getTime(), chidx, (unsigned long)freq);
#endif
            }
        }
    }
...
static void engineUpdate (void) {
#if LMIC_DEBUG_LEVEL > 0
    printf("%lu: engineUpdate, opmode=0x%x\n", (unsigned long int) os_getTime(), LMIC.opmode);
#endif
